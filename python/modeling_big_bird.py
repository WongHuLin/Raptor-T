...
class BigBirdBlockSparseAttention(nn.Module):
    ...
    def bigbird_block_sparse_attention(
        ...
        np.random.seed(seed)
        if from_seq_len % 256 == 0:  # old plans used in paper
            rand_attn = [
        ...
        if test_info is not None and "pre_process" in test_info:
            test_info["pre_process"] += (end - begin).total_seconds()
        ...
        if test_info is not None and "attention" in test_info:
            test_info["attention"] += (end - begin).total_seconds()
...
class BigBirdAttention(nn.Module):
    ...
    def forward(
        ...
        if self.attention_type == "original_full":
            ...
            self_outputs = self.self(
                hidden_states, band_mask, from_mask, to_mask, from_blocked_mask, to_blocked_mask, output_attentions,test_info = test_info,
            )
