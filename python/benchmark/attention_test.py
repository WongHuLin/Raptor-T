...
def attention_test(args):
    ...
    if model_name == 'raptor_t':
        ...
        for i in range(3):
            ...
            output,test_info = bertModel(hidden_states, total_seq_len,thread_block_limit, seq_position_info, seq_position_info_tensor,  partition_part_index_tensor,  partition_part_tensor, attention_masks, token_type_ids, position_ids,output_layer_temp,intermediate_temp,attention_output,test_info,balanced = balanced)

        for balanced in [True,False]:
            total_time = datetime.timedelta()
            test_info = {"attention":0.0}
            for i in range(20):
                metadata.update_meta_data(total_seq_len,seq_position_info[-1]+1,total_comp_block_num,seq_position_info)
            ...
            print("{} {} {} {} {} {} {}".format(model_name,batch_size,max_seqlen,seq_len,balanced,total_time.total_seconds()/20*1000,test_info["attention"]/20/1000))
