...
def kernel_fusion_mem_op(args):
    ...
    test_info = {"add_bias_and_layernorm_kernel":0.0,       
                "add_bias_act_kernel":0.0,
                "add_bias_and_transpose_kernel":0.0,}
    for i in range(3):
        metadata.update_meta_data(total_seq_len,seq_position_info[-1]+1,total_comp_block_num,seq_position_info)
        output,test_info = bertModel(hidden_states, total_seq_len,thread_block_limit, seq_position_info, seq_position_info_tensor,  partition_part_index_tensor,  partition_part_tensor, attention_masks, token_type_ids, position_ids,output_layer_temp,intermediate_temp,attention_output,test_info,kernel_fusion=kernel_fusion)
    ...
    for i in range(20):
        ...
        mem_bound_op_info.append(test_info)
    ...
    for i in range(len(total_time_info)):
        ...
        memory_op_result.append(mem_bound_op_info[i])

    non_memory = 0.0
    for value in memory_op_result:
        for v in value.values():
            non_memory += v
        

    print("{} {} {} {} {} {} {}".format(model_name,batch_size,max_seqlen,seq_len,kernel_fusion,np.mean(total_time_result),non_memory/len(memory_op_result)/1000))
