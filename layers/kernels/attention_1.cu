...
// (seq_len)(32*8)
// template <class DataType>
// __global__ void add_bias_and_layernorm(DataType *input_data, DataType *bias, int seq_len, int handle_row, int normalized_len, float eps, DataType *layernorm_weight, DataType *layernorm_bias){
//     const int tidy = threadIdx.y;
//     const int tidx = threadIdx.x;
//     const int bidx = blockIdx.x;
//     const int bidy = blockIdx.y;
...
//     __shared__ float bias_smem[768];
//     __shared__ float layernorm_weight_smem[768];
...
//     // load input 
//     auto block = cooperative_groups::this_thread_block();
//     auto block_layer = cooperative_groups::this_thread_block();

//     int data_offset = bidx*normalized_len*handle_row;
//     cooperative_groups::memcpy_async(block, input_smem[0], input_data+data_offset, sizeof(float)*2*768);
//     // cooperative_groups::memcpy_async(block, bias_smem, bias, sizeof(float)*768);
//     // cooperative_groups::memcpy_async(block_layer, layernorm_weight_smem, layernorm_weight, sizeof(float)*768);
//     int aggregate = WarpReduce(temp_storage[warp_id]).Sum(thread_data);


//     cooperative_groups::wait(block);
//     float sum[2][2] = {0.0f, 0.0f};
//     __shared__ float sum_temp_smem[8];
//     for(int i=0;i<3;i++){
...
// void test_add_bias_and_layernorm(float *input_data, float *bias,int seq_len, int handle_row, int normalized_len, float eps,float* layernorm_weight,float* layernorm_bias){
//     cudaEvent_t start,stop;
//     cudaEventCreate( &start );
//     cudaEventCreate( &stop ) ;
//     cudaEventRecord( start, 0 ) ;
//     add_bias_and_layernorm<float><<<dim3(),dim3(32,8)>>>(input_data,bias,seq_len,handle_row,normalized_len,eps,layernorm_weight,layernorm_bias);
