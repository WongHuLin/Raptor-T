        ...
        for(int block_id_index=to_block_start;block_id_index<to_block_end;block_id_index++){
            ...
            for(int from_block_part_index = 0;from_block_part_index<block_size;from_block_part_index+=16){
                ...
                if(tidy == 1 && tidx < 16)
                {
                    ...
                    max_score[idx] = max_score[idx] > temp_smem[tidx]?max_score[idx]:temp_smem[tidx];
                    max_score_diff[tidx] = exp(pre_max_score[idx]-max_score[idx]);
                    // printf("%f %f\n",pre_max_score[idx]-max_score[idx],max_score_diff[tidx]);
                }
                __syncthreads();


                half2 max_s = __half2half2(__float2half(max_score[tidx/8]));
                float4 value = FLOAT4(smem_temp_score[tidy*4+(tidx*8)/64][(tidx*8)%64]);
                half2 diff_x = _half2half2(__float2half(max_score_diff[tidx/8]));
                float4 out_t = FLOAT4(out_temp[from_block_part_index + tidy*4+(tidx*8)/64][(tidx*8)%64]);

                for(int i=0;i<4;i++){
                    half2 t = h2exp(__hsub2(HALF2(value)[i],max_s));
                    HALF2(smem_temp_score[tidy*4+(tidx*8)/64][(tidx*8)%64])[i] = t;
                    float v = __half2float(__hadd(HALF(t)[0],HALF(t)[1]));
                    temp_smem[tidy + i*4] = WarpReduce(temp_storage[tidy]).Sum(v);
