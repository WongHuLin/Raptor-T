...
//     int seq_len = 4096+1024, d_num = 768;
...
//     torch::Tensor seq_len_info = torch::zeros({3},torch::kInt);
...
//     reinterpret_cast<int*>(seq_len_info.data_ptr())[0] = 0;
//     reinterpret_cast<int*>(seq_len_info.data_ptr())[1] = 64;
//     reinterpret_cast<int*>(seq_len_info.data_ptr())[2] = 80;
...
//     test_add_bias_and_transpose(reinterpret_cast<float*>(bias.to(at::kCUDA).data_ptr()),reinterpret_cast<float*>(input_data.to(at::kCUDA).data_ptr()),reinterpret_cast<float*>(query.data_ptr()),reinterpret_cast<float*>(key.data_ptr()),reinterpret_cast<float*>(value.data_ptr()),0,d_num,d_num*2,reinterpret_cast<int*>(seq_len_info.to(at::kCUDA).data_ptr()),2,12,64,block_num,head_size);

//     // test_add_bias_and_transpose__(reinterpret_cast<float*>(bias.to(at::kCUDA).data_ptr()),reinterpret_cast<float*>(input_data.to(at::kCUDA).data_ptr()),reinterpret_cast<float*>(query.data_ptr()),reinterpret_cast<float*>(key.data_ptr()),reinterpret_cast<float*>(value.data_ptr()),0,d_num,d_num*2,1,4096,12,64,64,64);
...
//     auto query_1 = input_data.index({torch::indexing::Slice(4096, 4096+1024),torch::indexing::Slice(0, d_num)}).reshape({16,block_size,head_num,head_size}).permute({2,0,1,3}).transpose(-2,-1).contiguous();
//     // std::cout<<query.index({torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 20),torch::indexing::Slice(0, d_num)})<<std::endl;
//     // std::cout<<query_1.index({torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 1)})<<std::endl;
//     // std::cout<<query.index({torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 1)})<<std::endl;
//     // auto key_1 = input_data.index({torch::indexing::Slice(4096, 4096+1024),torch::indexing::Slice(d_num, d_num*2)}).reshape({16,block_size,head_num,head_size}).permute({2,0,1,3}).contiguous();
...
//     std::cout<<"The result is "<<check_value<float>(reinterpret_cast<float*>(query_1.to(at::kCPU).data_ptr()),reinterpret_cast<float*>(query.index({torch::indexing::Slice(4096, 4096+1024),"..."}).to(at::kCPU).data_ptr()),query_1.numel(),query_1.numel())<<std::endl;
...
//     torch::Tensor out_1 = out_data + input_data + bias;
...
//     std::cout<<out_1.to(at::kCPU).index({torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 20)})<<std::endl;
//     std::cout<<out_data.to(at::kCPU).index({torch::indexing::Slice(0, 1),torch::indexing::Slice(0, 20)})<<std::endl;

//     std::cout<<"The result is "<<check_value<float>(reinterpret_cast<float*>(out_1.to(at::kCPU).data_ptr()),reinterpret_cast<float*>(out_data.to(at::kCPU).data_ptr()),input_data.numel(),out_data.numel())<<std::endl;
...
void test(){
    torch::Tensor a = torch::arange(16*16,torch::kFloat32).reshape({16,16}).to(torch::kCUDA).toType(torch::kHalf).div(256);
    torch::Tensor out_2 = torch::zeros_like(a).reshape({16,16}).to(torch::kCUDA).toType(torch::kHalf);
    torch::Tensor out_1 = torch::mm(a,a);
    // std::cout<<out_1<<std::endl;
    test_wmma(reinterpret_cast<half*>(a.contiguous().data_ptr()),reinterpret_cast<half*>(a.contiguous().data_ptr()),reinterpret_cast<half*>(out_2.contiguous().data_ptr()));
    // std::cout<<out_2<<std::endl;
}

void test_stage1(){
    torch::Tensor a = torch::arange({64*64},torch::kFloat32).reshape({64,64}).to(torch::kCUDA).toType(torch::kHalf).div(64*64);
    torch::Tensor b = torch::arange({64*1024},torch::kFloat32).reshape({1024,64}).to(torch::kCUDA).toType(torch::kHalf).div(128*64);

    torch::Tensor out = torch::zeros({64,64}).to(torch::kCUDA).toType(torch::kHalf);
    torch::Tensor seq_len_info = torch::zeros({2},torch::kInt);
    reinterpret_cast<int*>(seq_len_info.data_ptr())[0] = 0;
    reinterpret_cast<int*>(seq_len_info.data_ptr())[1] = 1;

    torch::Tensor from_select_index_position = torch::zeros({2},torch::kInt);
    reinterpret_cast<int*>(from_select_index_position.data_ptr())[0] = 0;
    reinterpret_cast<int*>(from_select_index_position.data_ptr())[1] = 1;

    torch::Tensor from_select_index = torch::zeros({1},torch::kInt);
    reinterpret_cast<int*>(from_select_index.data_ptr())[0] = 0;

    torch::Tensor to_select_index_position = torch::zeros({2},torch::kInt);
    reinterpret_cast<int*>(to_select_index_position.data_ptr())[0] = 0;
    reinterpret_cast<int*>(to_select_index_position.data_ptr())[1] = 16;

    torch::Tensor to_select_index = torch::zeros({2},torch::kInt);
    for(int i=0;i<16;i++){
        reinterpret_cast<int*>(to_select_index.data_ptr())[i] = i;
    }

    seq_len_info = seq_len_info.to(torch::kCUDA);
    from_select_index_position = from_select_index_position.to(torch::kCUDA);
    from_select_index = from_select_index.to(torch::kCUDA);
    to_select_index_position = to_select_index_position.to(torch::kCUDA);
    to_select_index = to_select_index.to(torch::kCUDA);

    sparse_transformers::layers::kernels::sparse_attention_lastest_larunch(reinterpret_cast<half*>(a.data_ptr()),reinterpret_cast<half*>(b.data_ptr()),reinterpret_cast<half*>(b.data_ptr()),reinterpret_cast<half*>(out.data_ptr()),1,reinterpret_cast<int*>(seq_len_info.data_ptr()),reinterpret_cast<int*>(from_select_index.data_ptr()),reinterpret_cast<int*>(from_select_index_position.data_ptr()),reinterpret_cast<int*>(to_select_index.data_ptr()),reinterpret_cast<int*>(to_select_index_position.data_ptr()),1,64,12,64,64);
    auto out1 = torch::mm(a.toType(torch::kFloat32),b.transpose(0,1).toType(torch::kFloat32));
    
    // std::cout<<out1.index({"...",torch::indexing::Slice(64,128)})<<std::endl;

    auto temp_score = out1.softmax(-1);
    // std::cout<<temp_score<<std::endl;
    out1 = torch::mm(temp_score,b.toType(torch::kFloat32));
    // std::cout<<out1<<std::endl;


    // std::tuple<torch::Tensor, torch::Tensor> max_test = torch::max(out1,-1);
    // auto max_val = std::get<0>(max_test).unsqueeze(-1);;
    // out1 = torch::exp(out1 - max_val);
    // out1 = torch::mm(out1,b.toType(torch::kFloat32));
    ...
    // std::cout<<b.index({torch::indexing::Slice(0,128),torch::indexing::Slice(0,1)})<<std::endl;
    // std::cout<<out1<<std::endl;
...
void test_sparse_attention_test_larunch(){
    ...
    sparse_transformers::layers::kernels::sparse_attention_lastest_larunch(reinterpret_cast<half*>(query.data_ptr()),reinterpret_cast<half*>(key.data_ptr()),reinterpret_cast<half*>(value.data_ptr()),reinterpret_cast<half*>(out.data_ptr()),1,reinterpret_cast<int*>(seq_len_info_tensor.data_ptr()),reinterpret_cast<int*>(from_select_index_tensor.data_ptr()),reinterpret_cast<int*>(from_select_index_position_tensor.data_ptr()),reinterpret_cast<int*>(to_select_index_tensor.data_ptr()),reinterpret_cast<int*>(to_select_index_position_tensor.data_ptr()),160,64,12,block_size,head_size);
    // sparse_transformers::layers::kernels::sparse_attention_test_larunch(reinterpret_cast<half*>(query.data_ptr()),reinterpret_cast<half*>(key.data_ptr()),reinterpret_cast<half*>(value.data_ptr()),reinterpret_cast<half*>(out.data_ptr()),1,reinterpret_cast<int*>(seq_len_info_tensor.data_ptr()),reinterpret_cast<int*>(from_select_index_tensor.data_ptr()),reinterpret_cast<int*>(from_select_index_position_tensor.data_ptr()),reinterpret_cast<int*>(to_select_index_tensor.data_ptr()),reinterpret_cast<int*>(to_select_index_position_tensor.data_ptr()),block_size,head_size);
...
int main()
{
    sparse_transformers::layers::kernels::test_stage1();
    return 0;
}