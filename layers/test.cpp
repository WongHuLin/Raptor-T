...
void test_sparse_attention(){
    ...
    // torch::Tensor to_select_index_,to_select_index_position_;
    ...
    torch::Tensor out_1 = torch::zeros({4096,d_num},torch::kFloat).to(at::kCUDA);
    ...
    std::cout<<"The first result is "<<check_value<float>(reinterpret_cast<float*>(out.index({torch::indexing::Slice(0,4096),"..."}).reshape({head_num,64,block_size,d_num/head_num}).to(at::kCPU).contiguous().data_ptr()),reinterpret_cast<float*>(out_1.to(at::kCPU).contiguous().data_ptr()),out_1.numel(),out_1.numel())<<std::endl;
}
